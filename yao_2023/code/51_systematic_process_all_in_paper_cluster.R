...
residual_subset <- readRDS(file.path(work_directory, 
...
clustering_file <- file.path(work_directory, "yao_2023/data/module_list_df_2000_genes.csv")
...
dir.create(file.path(work_directory, 
                     'yao_2023/data/intermediate_data/51_pairwise/'), 
           recursive = TRUE, showWarnings = FALSE)
dir.create(file.path(work_directory, 'yao_2023/log/'), 
           recursive = TRUE, showWarnings = FALSE)
...
log_file <- file.path(work_directory, 
                      'yao_2023/log/logs.txt')
...
summary_results <- foreach(i = 1:nrow(treatment_pairs), .combine = rbind, .packages = c("data.table", "glue", "grpreg", "HMC")) %dopar% {
  ...
  output_filename <- file.path(work_directory, 
                               'yao_2023/data/intermediate_data/51_pairwise/', 
                               treatment1_name, "_", treatment2_name, '.rds')
  saveRDS(test_result, file = output_filename)
  
  return(data.table(treatment1 = treatment1_name, 
...
fwrite(summary_results, 
       file.path(work_directory, 'yao_2023/data/intermediate_data/51_pairwise/all_results_summary.csv'))
